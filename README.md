...
- **Occupancy Grid** — real-time top-down (bird's-eye) 2D map generated by projecting the 3D point cloud onto the XZ plane; occupied cells are dark gray, the trajectory is blue, with a green dot at the start and red dot at the current position. The grid auto-scales using the 1st–99th percentile of the point distribution to prevent extreme triangulation outliers from compressing the visible map.
